...
public class MapPanel extends JPanel  {
	 ...
	 */
	private static final long serialVersionUID = 2760754033162616622L;
	/**
		...
		for (int i = 0; i < rows; i++) {
			for (int j = 0; j < cols; j++) {
				tiles[i][j] = GUITile.createGUITile(terrainIconFiles.get(Game
						.getGame().getMap().getTile(i, j).getTerrainType()));
				tiles[i][j].setTile(Game.getGame().getMap().getTile(i, j));
				add(tiles[i][j]);
		...
		for (int i = 0; i < rows; i++) {
			for (int j = 0; j < cols; j++) {
				tiles[i][j] = GUITile.createGUITile(terrainIconFiles.get(Game
						.getGame().getMap().getTile(i, j).getTerrainType()));
				tiles[i][j].setTile(Game.getGame().getMap().getTile(i, j));
				if ((i >= offsetX) && (i < visibleRows + offsetX)
						&& (j >= offsetY) && (j < visibleCols + offsetY)) {
					panel.add(tiles[i][j]);
				}
				tiles[i][j].addMouseListener(TileListener.createListener(
						tiles[i][j], this));
	...
	private boolean tileIsVisible(final int iCoord, final int jCoord) {
		return iCoord >= offsetX && iCoord < visibleRows + offsetX && jCoord >= offsetY
				&& jCoord < visibleCols + offsetY;
	}
